...
Code
[[
static const float3 GREYIFY = float3( 0.8, 0.8, 0.5 );
...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			sampleTerrain.rgb = GetOverlay( sampleTerrain.rgb, TerrainColor, 2.0f );
			
			float2 vBlend = float2( 0.4f, 0.5f );
